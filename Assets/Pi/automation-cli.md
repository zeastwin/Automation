...
这是当前 EW-AI 进程的 Automation 路由上下文。所有平台工具经 Pi 内置 bash 工具执行 Automation.ToolCli.exe 调用。字段、枚举、运行语义、资源状态和合法迁移以本轮 cli 返回的 Schema、Guide 与结构化 JSON 为准。

## 工具调用机制

- 首次执行 `cli call` 前，先用 read 工具读取 `automation-tools-cli` Skill 的 SKILL.md 完整内容：参数 JSON 的 bash 引用写法、返回结构、退出码和大输出处理以该 Skill 为准。
- 可执行文件路径在环境变量 `AUTOMATION_TOOLCLI_PATH`；当前工具范围由环境变量 `AUTOMATION_TOOL_PROFILE` 决定。
- 调用工具：`"$AUTOMATION_TOOLCLI_PATH" cli call <name> --json '<argsJson>'`（单引号包裹、内部双引号不转义）；大体积参数（如 ChangeSet）写入 UTF-8 文件后用 `--json-file <path>`。
- 发现工具：`cli list`（名称与描述）；读取单个工具的精确输入契约：`cli schema <name>`。

## 任务入口

- 创建、修改、重构、复制或评审 `Proc -> Step -> OperationType` 流程时，先用 read 工具读取 `automation-process-authoring` Skill 的 SKILL.md 完整内容，再按该 Skill 的分阶段工作流执行。Skill 提供方法，不替代平台事实；其中出现的工具名均按上述 cli call 机制调用。
