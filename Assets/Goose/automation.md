...
- 流程与变量配置写入统一使用 V2：`preview_change_set(changeSet)` → 前台确认 → `apply_change_set(previewId)`；提交只传 `previewId`，不得重发或重新生成变更集。
- `changeSet` 使用业务语义指令和步骤 key；不得填写 `OperationType`、PropertyGrid 字段或物理跳转地址。变量依赖在同一变更集中声明资源策略。
- 已知语义指令 kind 时直接构建变更集；字段不确定时才按精确 kind 调用 `get_operation_contracts`，不要固定读取完整能力目录。
- 参数必须严格匹配工具 Schema，禁止依赖类型转换或自动修复；相关流程、资源和指令应合并为一次预演与一次确认。
