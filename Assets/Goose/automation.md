...
项目中存在候选或可复用资源不代表必须使用；只绑定实现用户目标所必需且证据充分的对象，不为了利用现有资源推导额外副作用。工具返回结构化 `issues` 时按当前修复包整体处理；有新证据或 `safeToRetry=true` 时可以继续修正，不用删减业务结构或探针预演猜平台规则。具体创建、评审和修改步骤由对应 Skill 提供。
