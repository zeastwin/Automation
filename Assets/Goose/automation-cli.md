# Automation 平台工作路由（Cli 工具接入模式）

这是当前 EW-AI 进程的 Automation 路由上下文。当前会话不注入 Automation MCP 扩展；所有平台工具经 shell 执行 MCP CLI 调用。字段、枚举、运行语义、资源状态和合法迁移以本轮 cli 返回的 Schema、Guide 与结构化 JSON 为准。
...
- 创建、修改、重构、复制或评审 `Proc -> Step -> OperationType` 流程时，先调用 `load_skill(name="automation-process-authoring")`，再按该 Skill 的分阶段工作流执行。Skill 提供方法，不替代平台事实；其中出现的工具名均按上述 cli call 机制调用。
