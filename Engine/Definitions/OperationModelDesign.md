...
- `OperationBehaviorCatalog` 已覆盖全部 52 种注册指令，并只为 TCP、串口和 PLC 数据通信指令投影固定间隔重试及接收结果判定。AI 读取原生 Schema 时同时得到用途、执行顺序、控制流、通信失败分类、条件字段、约束和失败模式，不需要从属性名猜测运行语义。`配置占位` 是设计期不可执行指令，可表达计划出口，但始终由 Readiness 阻止启动。
...
| 原生指令行为缺失 | 52 种指令已全部覆盖 | `OperationBehaviorCatalog` |
