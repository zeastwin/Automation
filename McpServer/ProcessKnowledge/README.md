...
- 一级检索维度是 `topics`，用于让 `get_process_design_guide` 按当前目标返回有界知识块；`capabilities` 描述块的核心能力，供返回后选型与关联，不冒充当前未实现的查询条件。设备和工艺只是多值标签，不建立额外树形目录；真实差异直接写入规范的"当前事实与适配"章节，出现实际复用压力后再建立新抽象。

## 块结构增强（20260829 批次）

在原有"规则型"结构上补充四个正交维度，全部寄宿在现有块与框架内，不新增获取机制。功能块的 compact 投影直接返回反模式与存在的定量惯例；设备框架首屏仍保持简索引，按 patternId 钻取后返回反模式与黄金样例：
