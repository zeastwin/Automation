...
sequenceDiagram
    ...
    Bootstrap->>Bootstrap: 加载 AppConfig / 校验启动参数
...
5. `PlatformRuntimeComposer` 创建真实运动控制、`EngineContext`、`ProcessEngine`、手动运动和设备协调器，并回填同一个 `PlatformRuntime`。
6. `PlatformRuntimeInitializer` 恢复未完成事务，加载流程、变量、运动卡、IO、工站、数据结构、通讯、报警和 PLC；随后由 `PlatformDeviceCoordinator` 初始化运动卡和轴监视，由 `PlatformSystemStatusService` 维护状态变量。
...
| --- | --- | --- | --- |
| HMI | 内置或设备工程 HMI | HMI 显示并进入空闲后，在同一 UI 线程隐藏创建窗口句柄并分片预热缓存；用户唤出前保持不可见 | 隐藏预加载不启动；打开平台编辑器或 AI 页面时按需启动 |
...
账户会话同样属于单个平台实例，由 `IAutomationPlatform.Authentication` 暴露登录、退出、当前账户和权限校验。平台编辑器窗口和工作区内容始终允许打开、查看；未登录或缺少 `platform.editor.open` 时只禁用受控入口并在执行边界拒绝操作，工具栏登录入口始终可用，避免“必须进编辑器登录、又因未登录打不开编辑器”的循环。退出登录不停止自动流程，但会停止手动运动并取消未保存的编辑草稿。`DEBUG` 构建在 HMI 初始化时通过 SDK 使用默认 `system` 账户自动登录，方便开发调试；发布构建不包含该自动登录语句。
...
- 已有值格式错误或越界时不猜测修复；平台尽量进入 HMI 或编辑器登录外壳，受影响能力降级并报警。
