...
平台仓库入口是 [`Program.cs`](../../Program.cs)，独立设备工程入口是 `../DeviceProject/Program.cs`。两者都先准备配置，再创建 `AutomationPlatformHost`。差异只在最终显示平台编辑器还是各自的 HMI；流程内核、配置目录和安全规则仍属于同一个宿主实例。
...
2. Pi 或 Git 不可用只产生 EW-AI 辅助能力降级，不阻断 HMI 和流程平台初始化。
...
| PlatformEditor | `FrmMain` | 启动视图确定后创建并附加已初始化运行时 | `FrmMain.Shown` 后按需启动 Bridge 与 AI 基础设施 |
...
2. 关闭断点、诊断和性能窗口。
3. 先释放 Pi 客户端，再停止 Bridge，避免 UI 同步调用与子进程读取线程互锁。
