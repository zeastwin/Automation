...
| 设备拓扑孪生 | `FrmEquipmentTopologyTwin` 承载 WebView 编辑与时间线回放；`TopologyRuleInferenceService` 按指令类型、参数和控制流生成候选；`EquipmentTopologyStore` 校验并原子提交 |
...
设备状态历史以 `EquipmentStateHistoryService` 的全局顺序时间线为事实源，`EquipmentStatePerceptionService` 只轮询已确认拓扑绑定的 IO，并把命令反馈、现场反馈和节点语义变化写入时间线。节点当前状态和历史时刻状态都是时间线投影，不按节点各自维护互不一致的历史。页面关闭后感知仍由平台运行时持有；页面仅查询最近窗口、基线快照并在前端回放，不参与采集，也不发出设备动作。时间线异步写入 `D:\AutomationLogs\StateHistory\yyyy-MM-dd\equipment-state.jsonl`，写盘失败不改变流程与设备状态。
