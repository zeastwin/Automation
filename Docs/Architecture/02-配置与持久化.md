...
| `AiConfig.json` | `AiConfigStorage` | Bootstrap、AI 基础设施 | Pi 子进程装配、AI 前台 |
| `Work/*.json` | `ProcessDefinitionStore.Items` | `ProcessConfigStore.Load/Rebuild`、`ProcessEditingService`、`AiConfigurationTransaction` | `ProcessEngine.Context.Procs` 的发布克隆 |
...
| `IODebugMap.json` | `IoDebugConfigurationStore` | `Load/TryCommit` | IO 调试页 |
| `value_debug.json` | `ValueDebugConfigurationStore` | `Load/TryCommit` | 变量调试页 |
...
| 类型 | 实际职责 |
| --- | --- |
| `ProcessDefinitionStore` | 当前可编辑流程定义的内存事实源；提供整体替换和深克隆快照。 |
| `ProcessConfigStore` | `Work/` 连续索引目录的整体重建、交换与断电恢复；它是目录事务工具。 |
| `ProcessEngineStore` | 对 `ProcessEngine` 的启动、暂停、继续、单步、停止和快照做窄接口投影；它不管理磁盘。 |
...
5. 更新 `ProcessDefinitionStore`，发布到 `ProcessEngine`。
6. 发布失败时恢复文件、正式内存和运行时；回滚不完整则进入安全锁。

### 结构重排或多对象提交

- 手工增删、复制和重排流程使用 `ProcessConfigStore.Rebuild`，通过 `Work_tmp`、`Work_bak` 和 `.complete` 做目录交换与启动恢复。
- AI ChangeSet 使用 `AiConfigurationTransaction` 和 `ConfigurationBatchWriter` 进行阶段化预演后的事务提交。
...
后续集中持久化所有权的具体工作见[技术债清单](07-技术债清单.md)。
