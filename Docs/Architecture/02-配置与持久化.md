...
| `AccountSecurity.json` | `AccountSecurityService` | `AccountSecurityStore` 严格加载并由管理员界面原子保存 | 登录会话和全部受控操作；不进入项目版本快照 |
...
| `EquipmentTopology.json` | `EquipmentTopologyStore` | 拓扑编辑页提交完整候选快照，Store 严格校验并原子保存 | 设备语义展示、实时状态映射、诊断上下文及 Machine Agent 动作前置校验；不替代控制器硬安全联锁 |
...
`AccountSecurity.json` 是安全配置而不是项目工艺配置。它保存 Schema、账户、级别、启用状态、完整权限清单以及每账户独立盐值的 PBKDF2-HMAC-SHA256 摘要参数；不保存明文或可解密密码。当前参数采用面向本地工控交互的 20,000 次轻量迭代，不兼容其他迭代参数。主文件与备份都损坏时认证失败关闭，不重新生成已知默认密码。该文件明确排除在 `ConfigurationVersionService` 快照之外，项目还原不会恢复旧密码、旧权限或已停用账户。

仅当主文件与备份都不存在时创建内置 `system` 系统管理员，初始密码为 `software_123`；一旦存在任何账户配置文件，加载失败也不得重新创建默认账户。内置账户只能由当前系统管理员修改密码，不能删除、停用、重命名、降级或取消权限。
