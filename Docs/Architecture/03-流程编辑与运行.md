...
“设置调试启动点”使用独立命令，不等同于重复启动：流程处于 `Ready/Stopped` 时创建新的单步实例；现有实例确实停在下一条指令前等待单步信号时，按稳定步骤/指令 ID 在同一 `runId` 内原地重定位。单步信号已经发出、指令正在执行或所选指令不属于当前运行修订时必须拒绝，不能把所有 `SingleStep` 快照视为可重定位状态。
