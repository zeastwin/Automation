...
“设置调试启动点”使用独立命令，不等同于重复启动：流程处于 `Ready/Stopped` 时创建新的单步实例；现有实例确实停在下一条指令前等待单步信号时，按稳定步骤/指令 ID 在同一 `runId` 内原地重定位。单步启动跳过整条流程的 Readiness、自定义函数扫描和完整运行时绑定，只在实际执行某条指令时绑定并校验该指令，因此未调试位置的不完整配置不会阻止单步。单步或暂停停留后切换“全速运行”时必须补做完整启动校验，通过后才能继续。单步信号已经发出、指令正在执行或所选指令不属于当前运行修订时必须拒绝，不能把所有 `SingleStep` 快照视为可重定位状态。
...
- 是否处于安全锁；
- 流程配置是否已故障；
- 配置版本还原后是否要求重启：仅编辑态分类（流程、变量、变量调试值、数据结构、工站、IO、报警）的还原按启动序列在内存重载并直接生效；控制卡、PLC、通讯、应用配置、AI 配置或 HMI 源码变更，以及内存重载失败时，设置重启闸门并要求完整重启。

### 单流程就绪

`ProcessReadinessService.Analyze` 检查当前流程是否存在启用的可执行指令，以及变量、报警、流程引用、跳转、通信重试与结果判定和指令运行必填项是否满足。其结果区分 `ready`、`incomplete`、`invalid`，并给出明确的 `RunBlockers`。该校验只阻断全速启动，以及从单步或暂停停留态切换到全速运行；单步调试不受整条流程 Readiness 阻断。
...
- `CT探针` 只在流程图的显式业务位置取样，使用单调时钟按当前 `runId + TaskKey` 隔离任务；对外结果统一使用秒并四舍五入到3位小数（0.001秒）。样本同步事件化并保留每个任务的最新值，不依赖位置快照或轮询，因此快速流程的开始和分段也不会丢失。最新值进入运行快照的 `cycleTimeSamples`，每次样本同时进入有界运行黑匣子；正常热路径不直接写盘。
- 单步调试在同一 `runId` 内重设启动点时保留已经开始的 CT 任务，因此可以执行计时起点后跳到后续分段；停止流程或创建新运行实例仍会清空本次 CT 状态，后续分段不能借用上一次运行的计时起点。
...
- `EquipmentProcessTimelineService` 只订阅上述既有生命周期事件，把流程启动、位置、指令失败和结束投影到全局设备状态时间线；它不插入执行热路径、不改变流程状态，也不替代运行黑匣子。Machine Agent 的已确认动作通过 `runId` 与这些事件关联，供后续按时间还原执行结果。
