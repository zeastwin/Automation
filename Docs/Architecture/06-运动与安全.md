...
- 实机模式使用 `MotionControl/MotionCtrl`，其下连接雷赛 LTDMC/LeiSei3000 适配实现。
- 仿真模式使用 `SimulationGatewayClient`，同时实现运动和 IO 接口。
- `FrmMain` 在构造阶段选择实现并注入 `EngineContext`、`ManualMotionService` 和 `PlatformRuntime`。
...
| 问题 | 入口 |
| --- | --- |
| 手动运动为什么被拒绝 | `Runtime/ManualMotionService.cs` |
| 实机轴状态校验 | `MotionControl/MotionCtrl.ValidateAxesForCommand` |
| 流程运动资源占用 | `ProcessEngine.TryAcquireMotionResources` 及 Motion operations |
| 运动卡实际适配 | `MotionControl/LeiSei3000.cs`、`MotionCtrl.InitCardType` |
| 轴监视故障 | `FrmMain.Monitor/HandleAxisMonitorFailure` |
| 全局安全锁 | `Runtime/PlatformLifecycleServices.cs` 中的 `PlatformSafetyCoordinator` |
