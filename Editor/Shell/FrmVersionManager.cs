...
<div class="shell">
  ...
  <div class="workspace">
    <aside class="sidebar">
      ...
      <div class="history-head"><span class="section-title">快照历史（最近 100 个）</span><span class="history-count" id="historyCount"></span></div>
...
<script>
var state={history:[],diff:[],variableDiff:[],dataStructDiff:[],summary:{}},restoreExpanded={};
...
function toggleRestoreGroup(key){restoreExpanded[key]=!restoreExpanded[key];renderHistory()}
function versionRow(v,child){var protection=v.snapshotType==='还原前保护点',restored=v.snapshotType==='还原结果',pattern=protection?/^还原前保护点[：:]?\s*/:restored?/^还原结果[：:]?\s*/:/^手动快照[：:]?\s*/,note=v.message.replace(pattern,'')||(protection?'还原前保护点':restored?'还原结果':'未填写说明'),prefix=protection?'<span style="color:#a15a2c">保护点 · </span>':restored?'<span style="color:#147b72">还原 · </span>':'';return '<div class="version'+(child?' child':'')+(v.commitId===state.selectedCommitId?' selected':'')+'" onclick="post(\'select\',{commitId:\''+esc(v.commitId)+'\'})"><div class="version-note">'+prefix+esc(note)+'</div><div class="version-meta"><span>'+esc(v.time)+'</span><span>'+esc(v.commitId.slice(0,8))+'</span></div></div>'}
// 还原产生的版本点（还原前保护点/还原结果）默认折叠为一个分组行，点击展开后逐条可选。
function renderHistory(){var all=state.history||[],rows=[],group=null,restoreCount=0;function flush(){if(group&&group.length){rows.push(group);group=null}}function isRestoreEntry(v){return v.snapshotType==='还原前保护点'||v.snapshotType==='还原结果'}all.forEach(function(v){if(isRestoreEntry(v)){if(!group)group=[];group.push(v)}else{flush();rows.push([v])}});flush();var html=rows.map(function(entries){entries.forEach(function(v){if(isRestoreEntry(v))restoreCount++});if(!(entries.length>=2&&entries.every(isRestoreEntry))){return entries.map(function(v){return versionRow(v,false)}).join('')}var key=entries[0].commitId,expanded=!!restoreExpanded[key]||entries.some(function(v){return v.commitId===state.selectedCommitId}),result=entries.filter(function(v){return v.snapshotType==='还原结果'})[0],target=result?result.message.replace(/^还原结果[：:]\s*/,''):'还原操作';return '<div class="version group-row" onclick="toggleRestoreGroup(\''+esc(key)+'\')"><div class="version-note"><span class="group-mark">'+(expanded?'▾':'▸')+'</span><span style="color:#147b72">还原操作</span><span class="group-count-inline">共 '+entries.length+' 个版本点</span><span class="group-target">→ '+esc(target)+'</span></div><div class="version-meta"><span>'+esc(entries[0].time)+'</span><span>'+esc(entries[0].commitId.slice(0,8))+'</span></div></div>'+(expanded?entries.map(function(v){return versionRow(v,true)}).join(''):'')}).join('');byId('historyCount').textContent=all.length+' 个'+(restoreCount?'（含 '+restoreCount+' 个还原点）':'');byId('history').innerHTML=html||'<div class="empty">当前还没有项目快照<br>填写说明后创建第一个快照</div>'}
