...
try {
    ...
    Wait-ProcessState -Engine $engine -State ([Automation.ProcRunState]::Ready)
...
try {
    ...
    Wait-ProcessState -Engine $publishEngine -State ([Automation.ProcRunState]::Ready)
    $readySnapshot = $publishEngine.GetSnapshot(0)
    Assert-True ($readySnapshot.PublishedRevision -eq $readySnapshot.AppliedRevision) '流程结束后内部版本状态应完成收敛。'
    Assert-True ($readySnapshot.ProcName -eq '下一运行版本') '后续运行应使用保存后的流程配置。'
}
finally {
...
try {
    ...
    Wait-ProcessState -Engine $maintenanceEngine -State ([Automation.ProcRunState]::Ready)
}
finally {
    if ($null -ne $lease) {
        $lease.Dispose()
...
try {
    ...
    Wait-ProcessState -Engine $ownerScopeEngine -State ([Automation.ProcRunState]::Ready)
    $privateAfterFirstRun = $valueStore.get_D_ValueByIndex(12)
    Assert-True ($ownerScopeEngine.StartProc($ownerScopeProc, 0)) '流程就绪后应允许再次启动。'
    Wait-ProcessState -Engine $ownerScopeEngine -State ([Automation.ProcRunState]::Ready)
...
$aiChangeSet.Actions = [System.Collections.Generic.List[Automation.Protocol.ChangeSetAction]]::new()
$createProcess = [Automation.Protocol.ChangeSetAction]::new()
$createProcess.Type = 'process.create'
$createProcess.Process = [Automation.Protocol.ProcessActionValue]::new()
$createProcess.Process.Key = 'ai_private_scope'
$createProcess.Process.Name = 'AI引用其他流程私有变量'
$aiChangeSet.Actions.Add($createProcess)
$appendStep = [Automation.Protocol.ChangeSetAction]::new()
$appendStep.Type = 'step.append'
$appendStep.TargetProcess = [Automation.Protocol.ProcessSelector]::new()
$appendStep.TargetProcess.Key = 'ai_private_scope'
$appendStep.Step = [Automation.Protocol.StepActionValue]::new()
$appendStep.Step.Key = 'main'
$appendStep.Step.Name = '主步骤'
$aiChangeSet.Actions.Add($appendStep)
$aiAdd = [Automation.Protocol.SemanticOperation]::new()
$aiAdd.Key = 'add_private'
$aiAdd.Kind = 'variable.add'
$aiAdd.Variable = '流程私有状态重命名'
$aiAdd.Amount = 1
$appendAdd = [Automation.Protocol.ChangeSetAction]::new()
$appendAdd.Type = 'operation.append'
$appendAdd.TargetProcess = [Automation.Protocol.ProcessSelector]::new()
$appendAdd.TargetProcess.Key = 'ai_private_scope'
$appendAdd.TargetStep = [Automation.Protocol.StepSelector]::new()
$appendAdd.TargetStep.Key = 'main'
$appendAdd.Operation = $aiAdd
$aiChangeSet.Actions.Add($appendAdd)
$aiEnd = [Automation.Protocol.SemanticOperation]::new()
$aiEnd.Key = 'end'
$aiEnd.Kind = 'flow.end'
$appendEnd = [Automation.Protocol.ChangeSetAction]::new()
$appendEnd.Type = 'operation.append'
$appendEnd.TargetProcess = [Automation.Protocol.ProcessSelector]::new()
$appendEnd.TargetProcess.Key = 'ai_private_scope'
$appendEnd.TargetStep = [Automation.Protocol.StepSelector]::new()
$appendEnd.TargetStep.Key = 'main'
$appendEnd.Operation = $aiEnd
$aiChangeSet.Actions.Add($appendEnd)
...
Assert-True ($aiVariableResolution.ownerProcName.ToString() -eq $createProcess.Process.Name) 'AI变量解析结果必须返回最终所属流程名称。'
...
Assert-True ([Automation.ProcessVariableLifecycleService]::ConvertOwnedVariablesToPublic(
    $copyVariables, [Guid[]]@($copyTargetProcId)) -eq 1) '删除流程并保留私有变量时应转换其作用域。'
Assert-True ($copiedVariable.Scope -eq [Automation.Protocol.VariableScopeContract]::Public) '保留的流程私有变量应转为公共变量。'
Assert-True (-not $copiedVariable.OwnerProcId.HasValue) '转为公共变量后不应保留已删除流程归属。'
$copiedVariable.Scope = [Automation.Protocol.VariableScopeContract]::Process
$copiedVariable.OwnerProcId = $copyTargetProcId
...
try {
    ...
    Wait-ProcessState -Engine $bindingEngine -State ([Automation.ProcRunState]::Ready)
