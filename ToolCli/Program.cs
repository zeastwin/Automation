...
namespace Automation.ToolCli
{
    internal static class Program
    {
        private static async Task<int> Main(string[] args)
        {
            ...
            // CLI 模式：把当前 Profile 的工具集作为命令行工具使用。
            ...
            Console.Error.WriteLine("仅支持 --verify-profile 或 cli 子命令，详见 cli --help。");
            return 2;
        }

        private static void VerifyEditorProfile()
        {
            IReadOnlyList<AiTool> editorTools = AiToolProfile.CreateEditorTools();
            HashSet<string> profiledToolNames = editorTools
                .Concat(AiToolProfile.CreateTools("Diagnostic"))
                .Concat(AiToolProfile.CreateTools("RuntimeDiagnostic"))
                .Concat(AiToolProfile.CreateTools("Editor", true))
                .Select(tool => tool.Name)
                .ToHashSet(StringComparer.Ordinal);
            string[] unprofiledDeclarations = typeof(AiPlatformTools)
                .GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static)
                .Select(method => method.GetCustomAttribute<AiToolAttribute>()?.Name)
            ...
            {
                throw new InvalidOperationException("工具源码包含未归属任何Profile的工具声明："
            ...
            AiTool flowGraphTool = editorTools.Single(tool =>
                string.Equals(tool.Name, "get_flow_graph", StringComparison.Ordinal));
            JsonObject? flowGraphSchema = JsonNode.Parse(flowGraphTool.InputSchema.GetRawText()) as JsonObject;
            ...
            AiTool listVariablesTool = editorTools.Single(tool =>
                string.Equals(tool.Name, "list_variables", StringComparison.Ordinal));
            JsonObject? listVariablesProperties = (JsonNode.Parse(
                listVariablesTool.InputSchema.GetRawText()) as JsonObject)?["properties"] as JsonObject;
            ...
            AiTool previewChangeSetTool = editorTools.Single(tool =>
                string.Equals(tool.Name, "preview_change_set", StringComparison.Ordinal));
            JsonObject? variableChangeSchema = FindSchemaByProperties(
                JsonNode.Parse(previewChangeSetTool.InputSchema.GetRawText()),
            ...
            AiTool addVariableTool = editorTools.Single(tool =>
                string.Equals(tool.Name, "add_variable", StringComparison.Ordinal));
            JsonObject? addVariableSchema = JsonNode.Parse(addVariableTool.InputSchema.GetRawText()) as JsonObject;
            ...
            if (addVariableIndexSchema?["minimum"]?.GetValue<int>() != 0
                ...
                || !(addVariableTool.Description ?? string.Empty).Contains(
                    "系统变量区配置对 AI 只读", StringComparison.Ordinal))
            {
                throw new InvalidOperationException("add_variable 未严格限制为普通变量区配置写入。");
            }
            AiTool updateVariableTool = editorTools.Single(tool =>
                string.Equals(tool.Name, "update_variable", StringComparison.Ordinal));
            JsonObject? updateVariableSchema = JsonNode.Parse(updateVariableTool.InputSchema.GetRawText()) as JsonObject;
            ...
            if (updateVariableProperties?["value"] == null
                ...
                || !(updateVariableTool.Description ?? string.Empty).Contains(
                    "value修改当前值", StringComparison.Ordinal))
            {
                throw new InvalidOperationException("update_variable 未使用单一当前值契约。");
            }
            AiTool deleteVariableTool = editorTools.Single(tool =>
                string.Equals(tool.Name, "delete_variable", StringComparison.Ordinal));
            AiTool getVariableByNameTool = editorTools.Single(tool =>
                string.Equals(tool.Name, "get_variable_by_name", StringComparison.Ordinal));
            AiTool getVariableByIndexTool = editorTools.Single(tool =>
                string.Equals(tool.Name, "get_variable_by_index", StringComparison.Ordinal));
            AiTool setVariableByNameTool = editorTools.Single(tool =>
                string.Equals(tool.Name, "set_variable_by_name", StringComparison.Ordinal));
            AiTool setVariableByIndexTool = editorTools.Single(tool =>
                string.Equals(tool.Name, "set_variable_by_index", StringComparison.Ordinal));
            JsonObject? getByNameProperties = (JsonNode.Parse(
                getVariableByNameTool.InputSchema.GetRawText()) as JsonObject)?["properties"] as JsonObject;
            JsonObject? getByIndexProperties = (JsonNode.Parse(
                getVariableByIndexTool.InputSchema.GetRawText()) as JsonObject)?["properties"] as JsonObject;
            JsonObject? setByNameProperties = (JsonNode.Parse(
                setVariableByNameTool.InputSchema.GetRawText()) as JsonObject)?["properties"] as JsonObject;
            JsonObject? setByIndexProperties = (JsonNode.Parse(
                setVariableByIndexTool.InputSchema.GetRawText()) as JsonObject)?["properties"] as JsonObject;
            if (!(updateVariableTool.Description ?? string.Empty).Contains(
                    "系统变量区配置对 AI 只读", StringComparison.Ordinal)
                || !(deleteVariableTool.Description ?? string.Empty).Contains(
                    "系统变量区配置对 AI 只读", StringComparison.Ordinal)
                ...
                || !(getVariableByNameTool.Description ?? string.Empty).Contains(
                    "私有变量也无需提供所属流程", StringComparison.Ordinal)
                || !(getVariableByIndexTool.Description ?? string.Empty).Contains(
                    "私有变量也无需提供所属流程", StringComparison.Ordinal)
                || !(setVariableByNameTool.Description ?? string.Empty).Contains(
                    "公共、私有和系统变量均可使用", StringComparison.Ordinal)
                || !(setVariableByIndexTool.Description ?? string.Empty).Contains(
                    "公共、私有和系统变量均可使用", StringComparison.Ordinal))
            {
            ...
            string[] pollutedDescriptions = editorTools
                .Where(tool => retiredRoutingTerms.Concat(ambiguousRoutingTerms).Any(term =>
                    (tool.Description ?? string.Empty).Contains(term, StringComparison.Ordinal)))
                .Select(tool => tool.Name)
                .ToArray();
            ...
            string[] pollutedSchemas = editorTools
                .Where(tool => retiredRoutingTerms.Concat(ambiguousRoutingTerms).Any(term =>
                    tool.InputSchema.ToString().Contains(term, StringComparison.Ordinal)))
                .Select(tool => tool.Name)
                .ToArray();
            ...
            AiTool previewTool = editorTools.First(tool =>
                string.Equals(tool.Name, "preview_change_set", StringComparison.Ordinal));
            string previewSchema = previewTool.InputSchema.ToString();
            ...
            int previewSchemaBytes = Encoding.UTF8.GetByteCount(
                previewTool.InputSchema.GetRawText());
            if (previewSchemaBytes > 64 * 1024)
            {
                throw new InvalidOperationException(
                    $"preview_change_set 参数Schema为{previewSchemaBytes}字节，超过64KB上下文预算。");
            }
            VerifyPreviewChangeSetDiscriminatedUnions(previewTool.InputSchema.GetRawText());
            VerifyDiagnosticPagingSchemas();
            AiTool runTestTool = editorTools.First(tool =>
                string.Equals(tool.Name, "run_proc_test", StringComparison.Ordinal));
            AiTool startTool = editorTools.First(tool =>
                string.Equals(tool.Name, "start_proc", StringComparison.Ordinal));
            AiTool discardPreviewTool = editorTools.First(tool =>
                string.Equals(tool.Name, "discard_change_set_preview", StringComparison.Ordinal));
            AiTool nativeSchemaTool = editorTools.First(tool =>
                string.Equals(tool.Name, "get_native_operation_schemas", StringComparison.Ordinal));
            AiTool semanticSchemaTool = editorTools.First(tool =>
                string.Equals(tool.Name, "get_semantic_operation_schema", StringComparison.Ordinal));
            AiTool processDesignTool = editorTools.First(tool =>
                string.Equals(tool.Name, "get_process_design_guide", StringComparison.Ordinal));
            AiTool getIoTool = editorTools.First(tool =>
                string.Equals(tool.Name, "get_io", StringComparison.Ordinal));
            AiTool getIoStateTool = editorTools.First(tool =>
                string.Equals(tool.Name, "get_io_state", StringComparison.Ordinal));
            if (!(runTestTool.Description ?? string.Empty).Contains("明确要求测试或试运行", StringComparison.Ordinal)
                || !(runTestTool.Description ?? string.Empty).Contains("负责启动、观察和安全停止", StringComparison.Ordinal)
                || !(startTool.Description ?? string.Empty).Contains("由run_proc_test一次完成", StringComparison.Ordinal)
                || !(discardPreviewTool.Description ?? string.Empty).Contains("不修改配置", StringComparison.Ordinal)
                || !(previewTool.Description ?? string.Empty).Contains("preview_only", StringComparison.Ordinal)
                || !(previewTool.Description ?? string.Empty).Contains("configurationSaved", StringComparison.Ordinal)
                || !(previewTool.Description ?? string.Empty).Contains("localKeyScope", StringComparison.Ordinal)
                || !(previewTool.Description ?? string.Empty).Contains("variableResolutions", StringComparison.Ordinal)
                || !(previewTool.Description ?? string.Empty).Contains("replacePreviewId", StringComparison.Ordinal)
                || !(nativeSchemaTool.Description ?? string.Empty).Contains("native.operation", StringComparison.Ordinal)
                || !(semanticSchemaTool.Description ?? string.Empty).Contains("保存必填项", StringComparison.Ordinal)
                || !(processDesignTool.Description ?? string.Empty).Contains("简单赋值", StringComparison.Ordinal)
                || !(processDesignTool.Description ?? string.Empty).Contains("不提供具体字段", StringComparison.Ordinal)
                || !(processDesignTool.Description ?? string.Empty).Contains("mechanical对应IO、气缸、真空和运动反馈", StringComparison.Ordinal)
                || !(processDesignTool.Description ?? string.Empty).Contains("review对应设计前、中、后审查", StringComparison.Ordinal)
                || !(getIoTool.Description ?? string.Empty).Contains("不自动定义机构的安全位或工作位", StringComparison.Ordinal)
                || !(getIoStateTool.Description ?? string.Empty).Contains("运行时逻辑状态", StringComparison.Ordinal)
                || !(getIoStateTool.Description ?? string.Empty).Contains("不统一表示电气高低电平、安全位或工作位", StringComparison.Ordinal))
            {
                throw new InvalidOperationException("预演生命周期或流程验证工具职责未完整公开。");
            }
            string semanticSchema = semanticSchemaTool.InputSchema.GetRawText();
            string nativeSchema = nativeSchemaTool.InputSchema.GetRawText();
            string processDesignSchema = processDesignTool.InputSchema.GetRawText();
            ...
            IReadOnlyList<AiTool> diagnosticTools = AiToolProfile.CreateTools("Diagnostic");
            string[] diagnosticNames = diagnosticTools.Select(tool => tool.Name).ToArray();
            ...
            AiTool diagnoseIssueTool = diagnosticTools.Single(tool =>
                string.Equals(tool.Name, "diagnose_issue", StringComparison.Ordinal));
            if (!(diagnoseIssueTool.Description ?? string.Empty).Contains(
                    "运行黑匣子", StringComparison.Ordinal)
                || !(diagnoseIssueTool.Description ?? string.Empty).Contains(
            ...
            string[] runtimeDiagnosticNames = AiToolProfile.CreateTools("RuntimeDiagnostic")
            ...
            string[] fullPermissionNames = AiToolProfile.CreateTools("Editor", true)
                .Select(tool => tool.Name).ToArray();
            ...
            {
                AiTool migrationTool = AiToolProfile.CreateTools("Editor", true).First(tool =>
                    string.Equals(tool.Name, contract.Key, StringComparison.Ordinal));
                string schema = migrationTool.InputSchema.GetRawText();
        ...
        {
            IReadOnlyList<AiTool> tools = AiToolProfile.CreateTools("Editor")
                .Concat(AiToolProfile.CreateTools("Diagnostic"))
                .Concat(AiToolProfile.CreateTools("RuntimeDiagnostic"))
        ...
        private static void VerifyNumericRange(
            ...
            int expectedMaximum)
        {
            AiTool tool = tools.Single(item =>
                string.Equals(item.Name, toolName, StringComparison.Ordinal));
            JsonObject root = JsonNode.Parse(tool.InputSchema.GetRawText()) as JsonObject
