...
{
    internal static class AiToolProfile
    {
        ...
        public static IReadOnlyList<AiTool> CreateTools(string profile, bool fullPermissionEnabled = false)
        {
            ...
            {
                throw new InvalidDataException($"工具Profile不支持:{profile}");
            }
                ...
                BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static))
            {
                ...
                AiTool tool = AiToolSchemaFactory.Create(method);
            ...
            if (tools.Count == 0)
            {
                throw new InvalidOperationException($"工具Profile未注册任何工具:{profile}");
            }
            return tools.OrderBy(tool => tool.Name, StringComparer.Ordinal).ToList();
        }

        private static void ApplySemanticKindSchema(AiTool tool)
        {
            ...
            {
                throw new InvalidOperationException($"{tool.Name} 参数Schema缺少字段：kind");
            }
        ...
        private static void ApplyStringArraySchema(
            AiTool tool, string propertyName, IEnumerable<string>? allowedValues)
        {
            JsonObject? root = JsonNode.Parse(tool.InputSchema.GetRawText()) as JsonObject;
            if (root?["properties"] is not JsonObject properties
                || properties[propertyName] is not JsonObject arraySchema)
            {
                throw new InvalidOperationException($"{tool.Name} 参数Schema缺少字段：{propertyName}");
            }
        ...
        private static void ApplyToolNumericRange(
            AiTool tool, string propertyName, int minimum, int maximum)
        {
            JsonObject? root = JsonNode.Parse(tool.InputSchema.GetRawText()) as JsonObject;
            if (root?["properties"] is not JsonObject properties
        ...
        private static void ApplyToolStringEnum(
            AiTool tool, string propertyName, params string[] allowedValues)
        {
            JsonObject? root = JsonNode.Parse(tool.InputSchema.GetRawText()) as JsonObject;
            if (root?["properties"] is not JsonObject properties
                || properties[propertyName] is not JsonObject propertySchema)
        ...
        private static void ApplyChangeActionDiscriminator(AiTool tool)
        {
            JsonObject? root = JsonNode.Parse(tool.InputSchema.GetRawText()) as JsonObject;
        ...
        public static IReadOnlyList<AiTool> CreateEditorTools()
        {
