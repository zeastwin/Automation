...
{
    /// <summary>
    /// 命令行入口：把当前 Profile 的工具集当作命令行工具使用。
    /// 发现与 Schema 读取按需输出（list/schema），调用（call）使用
    /// 同一批 [AiTool] 声明、同一份 Profile 集合和同一条 Bridge 管道链路。
    /// </summary>
    internal static class CliCommand
    {
        ...
        public static async Task<int> RunAsync(string[] args)
        {
            ...
            if (profile == null)
            {
                Console.Error.WriteLine(
                    $"工具Profile不支持:{profileOverride ?? Environment.GetEnvironmentVariable("AUTOMATION_TOOL_PROFILE")}，可选Editor/Diagnostic/RuntimeDiagnostic。");
                return ExitUsage;
            }

            bool fullPermission = args.Any(value => string.Equals(value, "--full-permission", StringComparison.OrdinalIgnoreCase))
                || IsTruthyEnvironment("AUTOMATION_TOOL_FULL_PERMISSION");
            if (fullPermission && !string.Equals(profile, "Editor", StringComparison.Ordinal))
            {
                // 完全权限只追加 Editor 的迁移/平台配置工具。
                Console.Error.WriteLine("完全权限只能在Editor模式下开启。");
                return ExitUsage;
            }

            IReadOnlyList<AiTool> tools = AiToolProfile.CreateTools(profile, fullPermission);
        ...
        private static string? ResolveProfile(string? profileOverride)
        {
            string candidate = !string.IsNullOrWhiteSpace(profileOverride)
                ...
                : Environment.GetEnvironmentVariable("AUTOMATION_TOOL_PROFILE")?.Trim() ?? string.Empty;
        ...
        private static void PrintToolList(IReadOnlyList<AiTool> tools, bool fullSchema)
        {
            ...
            foreach (AiTool tool in tools)
            {
                ...
                if (fullSchema)
                {
                    item["inputSchema"] = JsonNode.Parse(tool.InputSchema.GetRawText());
                }
                items.Add(item);
            }
        ...
        private static int PrintToolSchema(IReadOnlyList<AiTool> tools, string? toolName)
        {
            ...
            AiTool? tool = FindTool(tools, toolName);
            if (tool == null)
            {
            ...
            string? jsonArgument)
        {
            ...
            MethodInfo? method = FindToolMethod(tool.Name);
            if (method == null)
            {
                Console.Error.WriteLine($"工具缺少方法实现:{tool.Name}");
            ...
            try
            {
                ...
                string text = invocationResult switch
                {
                    ...
                    _ => throw new InvalidOperationException(
                        $"工具 {tool.Name} 返回类型不受支持:{method.ReturnType.Name}")
                };
        ...
        private static AiTool? FindTool(IReadOnlyList<AiTool> tools, string toolName)
        {
            foreach (AiTool tool in tools)
            {
        ...
        {
            foreach (MethodInfo method in typeof(AiPlatformTools).GetMethods(
        ...
        private static int PrintUnknownTool(IReadOnlyList<AiTool> tools, string toolName)
        {
            Console.Error.WriteLine($"当前Profile未开放工具:{toolName}");
            Console.Error.WriteLine("可用工具:" + string.Join(", ", tools.Select(tool => tool.Name)));
            return ExitUsage;
        }
        ...
        {
            Console.Error.WriteLine(
                "Automation ToolCli — 以命令行方式使用当前Profile的AI工具集。" + Environment.NewLine
                ...
                + "Profile 解析顺序：--profile 参数 > AUTOMATION_TOOL_PROFILE 环境变量 > Editor。" + Environment.NewLine
                + "完全权限解析顺序：--full-permission 参数 > AUTOMATION_TOOL_FULL_PERMISSION 环境变量（1/true）> 关闭。" + Environment.NewLine
        ...
        // 反序列化采用 Web 默认（camelCase、不区分大小写、字符串数字与字符串枚举），
        // 但未知字段立即报错：避免模型把 steps/ops 嵌套进
