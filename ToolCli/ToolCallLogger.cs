...
{
    /// <summary>
    /// AI 工具调用日志落盘，便于复盘用户问题、AI行为和工具结果。
    /// </summary>
    internal static class ToolCallLogger
    {
        ...
        private static readonly string logRoot = Path.Combine(@"D:\AutomationLogs", "AIExecution");
        ...
        {
            string toolCallId = CurrentCallId.Value ?? string.Empty;
            DateTime now = DateTime.UtcNow;
            WriteRecord(new
            {
                time = now.ToLocalTime().ToString("O"),
            ...
            }, new
            {
                ...
                eventId = Guid.NewGuid().ToString("N"),
                correlationId = string.IsNullOrWhiteSpace(toolCallId) ? bridgeRequestId : toolCallId,
                toolCallId,
