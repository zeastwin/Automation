...
- `FrmInfo` 运行信息页使用 `ReceiveTextBox` 显示信息流，字体为 `Calibri 9pt`；日志仅对时间头（`[yyyy-MM-dd HH时mm分ss秒]`）做级别染色，分隔符 `：` 与正文不染色；默认追加日志后自动滚动到底部，当用户上翻查看历史时会暂停自动滚动，空闲 `20s` 后恢复自动滚动；信息流按 `200` 条上限自动清除最旧记录，同时支持右键“清空”手动清除。
- 运行动作权限：
