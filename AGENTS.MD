...
- **运行引擎**：`ProcessEngine` 为每个流程开启线程，`ProcHandle` 记录索引、状态与运行标志。`RunProc`/`RunStep` 支持暂停、单步、跳转、报警；`ExecuteOperation` 将具体操作派发到对应执行函数。
...
- **流程调度**：`ProcessEngine` 为每个流程创建 `Thread`，通过同步门闩控制运行/暂停/单步。
